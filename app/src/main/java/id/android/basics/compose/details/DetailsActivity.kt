...
private fun MapViewContainer(
  ...
  AndroidView({ map }) { mapView ->
    // Reading zoom so that AndroidView recomposes when it changes. The getMapAsync lambda is stored for later, Compose doesn't recognize state reads
    val mapZoom = zoom
...
private fun ZoomControls(
  ...
  Row(
    ...
    ZoomButton(
      "-",
      onClick = { onZoomChanged(zoom * 0.8f) }
    )
    ZoomButton(
      "+",
      onClick = { onZoomChanged(zoom * 1.2f) }
    )
  }
}
